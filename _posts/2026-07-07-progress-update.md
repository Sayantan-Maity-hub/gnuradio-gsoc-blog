...
Flask exposes this registry through REST endpoints such as:
GET /status/nodes
...
Transfer the script through SFTP over the existing SSH connection.
Execute the script remotely.
Capture stdout and stderr.
Store execution results inside an in-memory execution registry.
Expose the execution state through REST APIs.
A simple demonstration script was used:
...
The next major objective is Milestone 4 – Experiment State Tracking.
Milestone 4 – Experiment State Tracking
Milestone-4: Experiment State Tracking
Goal:
Track experiment Progress. Make available under /status/…
Problem:
The Controller must know where an experiment currently is.
State:
CREATED
ASSIGNED
PREPARING
...
Demonstration:
Experiment 17: CREATED → ASSIGNED →PREPARING→READY→RUNNING→FINISHED
Estimated Effort:
8 - 12 Hours
